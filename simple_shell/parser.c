 ...
 *
 * Return: (Success) 0 is returned, (Fail) otherwise
 */
...
{
	char *path, *current_token, *_path;
	struct stat st;
	int exist_flag = 0;

	path = _getenv("PATH");
	_path = _strdup(path);
	current_token = strtok(_path, DELIMITER);
	while (current_token)
	{
		data->cmd = _strcat(current_token, data->args[0]);
 ...
 *
 * Return: (Success) 0 is returned, (Fail) negative number will returned
 */
